...
pub trait WebViewBuilderExtWindows {
  ...
  /// Set the WebView2 profile this webview runs under.
  ///
  /// Profiles are isolated storage partitions — cookies, localStorage, cache —
  /// *within a single user data folder*, which means they share one browser
  /// process tree. That is the whole point: two webviews that need separate
  /// cookie jars would otherwise need separate user data folders, and WebView2
  /// spawns a full set of browser, GPU, network and crashpad processes per
  /// folder.
  ///
