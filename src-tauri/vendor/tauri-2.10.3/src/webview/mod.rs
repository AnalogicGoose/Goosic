...
```
  ...
  /// Windows only. Run this webview in a named WebView2 profile.
  ///
  /// A profile is an isolated cookie and storage partition *inside* the shared
  /// user data folder. It is the counterpart to macOS's
  /// [`Self::data_store_identifier`]: separate sessions without separate data
  /// directories, and therefore without a second browser process tree — a
  /// distinct `data_directory` costs a whole set of browser, GPU, network and
  /// crashpad processes, while a profile costs one renderer.
  ///
  /// Ignored on every other platform.
  ///
  /// Names must satisfy WebView2's rules: alphanumeric plus `.`, `_`, `-` and
  /// space, at most 64 characters, not starting or ending with `.` or a space.
  #[must_use]
  pub fn profile_name(mut self, profile_name: impl Into<String>) -> Self {
    self.webview_attributes.profile_name = Some(profile_name.into());
    self
  }
