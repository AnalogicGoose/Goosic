...
impl<R: Runtime, M: Manager<R>> WebviewWindowBuilder<'_, R, M> {
  ...
  /// Windows only. Run this webview in a named WebView2 profile — an isolated
  /// cookie and storage partition inside the shared user data folder, so a
  /// separate session no longer costs a separate browser process tree.
  ///
  /// See [`WebviewBuilder::profile_name`]. Ignored on other platforms.
  #[must_use]
  pub fn profile_name(mut self, profile_name: impl Into<String>) -> Self {
    self.webview_builder = self.webview_builder.profile_name(profile_name);
    self
  }
