...
pub struct WebviewAttributes {
  ...
  /// Windows only. The WebView2 profile this webview runs under: an isolated
  /// cookie/storage partition *inside* the shared user data folder, so surfaces
  /// that need separate sessions no longer need separate data directories (and
  /// therefore separate browser process trees).
